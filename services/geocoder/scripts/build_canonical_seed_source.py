...
def _build_country_features(raw_features: list[dict[str, Any]]) -> tuple[list[dict[str, Any]], dict[str, list[str]]]:
    ...
    for feature in raw_features:
        ...
        if not isinstance(geometry, dict) or str(geometry.get("type")) not in {"Polygon", "MultiPolygon"}:
            continue
        props = feature.get("properties") or {}
        country_name = _first_nonempty(props, ["NAME_EN", "NAME", "ADMIN", "NAME_LONG", "SOVEREIGNT", "FORMAL_EN"])
        if not country_name:
            continue
...
def _build_region_features(raw_features: list[dict[str, Any]], country_aliases_by_adm0: dict[str, list[str]]) -> list[dict[str, Any]]:
    features: list[dict[str, Any]] = []
    for feature in raw_features:
        geometry = feature.get("geometry")
        if not isinstance(geometry, dict) or str(geometry.get("type")) not in {"Polygon", "MultiPolygon"}:
...
def build_seed_source_dataset(output_path: Path) -> dict[str, int]:
    ...
    output_path.write_text(
        json.dumps({"type": "FeatureCollection", "features": all_features}, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    return {
