...
`AGENT/INDEX.md` explains the order and routing. This kernel is the small, high-signal entry point for coding agents.
...
`TASKS/` remains available as scoped execution material, but it is no longer part of the default preload.
...
- global invariants and table/service boundaries -> `AGENT/PROJECT_CONSTITUTION.md`
- coding-agent loading and change behavior -> `AGENT/EXECUTION_MODEL.md`
- active repository phase context -> `AGENT/CURRENT_PHASE.md`
...
`AGENT/doc_router.yaml` provides a small static lookup table for task-to-doc routing. It adds no runtime dependency and is simple enough for autonomous agents to consume directly.
